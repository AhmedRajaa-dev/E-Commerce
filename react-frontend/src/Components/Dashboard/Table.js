...
export default function TableShow(props) {
  ...
  //header show
  const headerShow = props.header.map((el) => (
  ...
  const dataShow = props.data.map((el, key) => (
    <tr key={key}>
      ...
      {props.header.map((item, key2) => (
        <td key={key2} class="px-6 py-4">
          {item.key === "image" ? (
            <img className="w-14 h-14" src={el[item.key]} alt="" />
          ) : item.key == "images" ? (
            el[item.key].map((img) => (
              <img width="50px" src={img.image} alt="" />
            ))
          ) : item.key === "created_at" || item.key === "updated_at" ? (
            TransformDate(el[item.key])
          ) : el[item.key] === "1995" ? (
            "Admin"
          ) : el[item.key] === "2001" ? (
            "User"
          ...
          {currentUser && el[item.key] === currentUser.name && " (You)"}
        </td>
      ))}
      <td class="px-6 py-4 flex items-center gap-2">
        ...
        {currentUser.name !== el.name && (
          <FontAwesomeIcon
            ...
            icon={faTrashCan}
            color="red"
            onClick={() => props.handleDelete(el.id)}
          />
        )}
      </td>
    </tr>
          ...
          >
            ...
            >
              <li>
                <a
                  href="#"
                  class="block p-2 hover:bg-neutral-tertiary-medium hover:text-heading rounded-md"
                >
                  Shopping
                </a>
              </li>
