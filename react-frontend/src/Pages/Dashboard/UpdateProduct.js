            ...
            try {
                const res =await Axios.post(`/product-img/add`,data,{
                    onUploadProgress:(ProgressEvent)=>{
                       ...
                       progress.current[`text-${startIndex + i}`].innerText = `${percent}%`;
                    },
                })
     ...
     async function handleEdit(e){
        ...
        useEffect(()=>{
            ...
            const imagesShow=images.map((img,key)=>(
                <div key={key} className="border p-2 w-screen">
                    <div className="flex items-center justify-between gap-2 w-full">
                        ...
                        <div>
                            ...
                            <p>
                                {img.size /1024<900
                                ?(img.size /1024).toFixed(2)+ "KB":(img.size /(1024*1024)).toFixed(2)+ "MB"
                                }
                            </p>
                        </div>
                ...
                <span className="text-sm font-medium text-body"  ref={(e) => (progress.current[`text-${key}`] = e)}></span>
                </div>
                <div className="w-full bg-neutral-quaternary rounded-full h-2">
                <div className="bg-blue-700 h-2 rounded-full"ref={(e)=>(progress.current[key]=e)} style={{ width: "0%" }}></div>
                </div>
