...
export default function Category(){
    ...
    useEffect(()=>{
        Axios.get(`${CATEGORY}/${id}`).then((res)=>{
            ...
            setLaouding(true);
        }).then(()=>setDisable(false)).catch(()=>navigate("dashboard/Categories/error/404"))
        ...
        <div>
            <form className="max-w-sm mx-auto" onSubmit={handleSubmit}>
                <div className="mb-5">
                    ...
                    <input type="title" id="name" className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"  required  value={title}  onChange={(e) => setTitle(e.target.value)}/>
                </div>
